...
def traffic_multiplier(level: str) -> float:
    return float(TRAFFIC_LEVELS.get(level, TRAFFIC_LEVELS["Low"])["multiplier"])


def edge_travel_minutes(data: dict, speed_kmph: float) -> float:
    level = data.get("traffic", "Low")
    if level == "Blocked":
        return math.inf
    distance_km = float(data.get("distance_km", 1.0))
    return (distance_km / max(speed_kmph, 1.0)) * 60.0 * traffic_multiplier(level)


def _edge_weight(speed_kmph: float) -> Callable[[Node, Node, dict], float]:
    def weight(_u: Node, _v: Node, data: dict) -> float:
        return edge_travel_minutes(data, speed_kmph)

    return weight


def summarize_path(graph: nx.Graph, path: list[Node], speed_kmph: float) -> RouteResult:
    ...
    for u, v in zip(path[:-1], path[1:]):
        ...
        segment_minutes = edge_travel_minutes(data, speed_kmph)
...
def fastest_route(graph: nx.Graph, source: Node, target: Node, speed_kmph: float) -> RouteResult:
    """Shortest-time route using current traffic as edge weights."""
    try:
        path = nx.shortest_path(graph, source=source, target=target, weight=_edge_weight(speed_kmph), method="dijkstra")
    except (nx.NetworkXNoPath, nx.NodeNotFound):
        return RouteResult([], math.inf, math.inf, 4.0, 999)

    result = summarize_path(graph, path, speed_kmph)
    if not math.isfinite(result.minutes):
        return RouteResult([], math.inf, math.inf, 4.0, 999)
    return result


def distance_route(graph: nx.Graph, source: Node, target: Node, speed_kmph: float) -> RouteResult:
    ...
    try:
        ...
        return RouteResult([], math.inf, math.inf, 4.0, 999)
    return summarize_path(graph, path, speed_kmph)
...
def select_ambulance(
    ...
    for ambulance in ambulances:
        if not ambulance.is_idle:
            continue
        route = fastest_route(graph, ambulance.current_node, emergency.node, ambulance.speed_kmph)
        if not route.is_valid:
            continue
        normal = distance_route(graph, ambulance.current_node, emergency.node, ambulance.speed_kmph)
...
def select_hospital(
    ...
    for hospital in hospitals:
        if not hospital.can_accept:
            continue
        route = fastest_route(graph, source_node, hospital.node, speed_kmph)
        if not route.is_valid:
            continue
